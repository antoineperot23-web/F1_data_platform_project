...
def ingest_drivers():
    ...
    logger.info(f"📡 Récupération drivers info ...")
    response = requests.get(url)
    
    if response.status_code == 200:
        data = response.json()
        logger.info(f"✅ {len(data)} drivers")
        
        df = pd.DataFrame(data)
        engine = create_engine(DB_URL)
        
        df.to_sql("raw_drivers", engine, if_exists="replace", index=False)
        logger.info("💾 Sauvegardé dans raw_drivers")
